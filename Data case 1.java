...
import test.MersenneTwisterFast;
			...
			for (int j=0; j<nbmaxjob; j++) {
				...
				for (int l=0; l<nbLignesForJob[j]; l++) {
					...
					for (int op=0; op<nbOperationsOnLigneForJob[j][l]; op++) {
						...
						int dureeMax = 0;
						int dureeMin = 99999;
						int energyMaxMac = 0;int energyMinMac = 999999;
						for (int m=0; m<nbMachinesForOperationOnLigneForJob[j][l][op]; m++) {
							...
							if (energyMinMac>energyOnMachineForOperationOnLigneForJob[j][l][op][m]) energyMinMac=energyOnMachineForOperationOnLigneForJob[j][l][op][m];
							if (energyMaxMac<energyOnMachineForOperationOnLigneForJob[j][l][op][m]) energyMaxMac=energyOnMachineForOperationOnLigneForJob[j][l][op][m];
